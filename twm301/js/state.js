/**
 * TWM 301 Midterm Reviewer — State Machine & Session Management
 ...
 * - QuizSession model with Stratified & Fisher-Yates randomized question shuffling
...
(function (global) {
  ...
  const STORAGE_KEYS = Object.freeze({
    API_KEY: 'TWM301_GEMINI_API_KEY',
    SHUFFLE_PREF: 'TWM301_SHUFFLE_PREF',
    DIFFICULTY_PREF: 'TWM301_DIFFICULTY_PREF',
    AI_ENABLED: 'TWM301_AI_ENABLED',
    AI_CACHE: 'TWM301_AI_EXPLANATIONS_CACHE',
    AI_MODEL: 'TWM301_AI_MODEL'
  });
  ...
  function getApiKey() {
    const raw = safeGetItem(STORAGE_KEYS.API_KEY)
      || safeGetItem('MCS306_GEMINI_API_KEY')
      || safeGetItem('MAT304a_GEMINI_API_KEY')
      || safeGetItem('MCS305_GEMINI_API_KEY')
  ...
  function initSession(settings = {}) {
    ...
    // Stratified deck sizing across all 7 syllabus topics
    let targetCount = masterQuestions.length;
    if (difficulty === 'easy') {
      targetCount = Math.min(50, masterQuestions.length);
    } else if (difficulty === 'medium') {
      targetCount = Math.min(80, masterQuestions.length);
    } else {
      targetCount = masterQuestions.length;
    }

    let selectedIds = [];
    if (targetCount >= masterQuestions.length) {
      selectedIds = masterQuestions.map(q => q.id);
    } else {
      // Stratified proportional sampling across all 7 syllabus topics
      const topicBuckets = {};
      masterQuestions.forEach(q => {
        const top = q.topic || 'General';
        if (!topicBuckets[top]) topicBuckets[top] = [];
        topicBuckets[top].push(q);
      });
      const topics = Object.keys(topicBuckets);
      const perTopic = Math.floor(targetCount / topics.length);
      let remainder = targetCount % topics.length;

      topics.forEach(t => {
        const pool = shuffleArray([...topicBuckets[t]]);
        const take = perTopic + (remainder > 0 ? 1 : 0);
        if (remainder > 0) remainder--;
        pool.slice(0, take).forEach(item => selectedIds.push(item.id));
      });
    }

    // 1. Order of questions
    session.questionOrder = isQuestionShuffled ? shuffleArray(selectedIds) : selectedIds;
  ...
  function getSavedTheme() {
    try {
      if (typeof localStorage !== 'undefined') {
        const saved = localStorage.getItem('mat304a_theme');
  ...
  function toggleTheme() {
    ...
    try {
      if (typeof localStorage !== 'undefined') {
        localStorage.setItem('mat304a_theme', nextTheme);
      }
    } catch (e) {}
