/**
 * TWM 301 Midterm Reviewer — Mathematical Typesetting & LaTeX Engine
 ...
 * - LaTeX code token stashing and inline formatting for commands and environments
...
(function (global) {
  ...
  function render(input) {
    ...
    // Stash inline LaTeX code snippets `...` (e.g. `\documentclass{article}`, `amsmath`)
    safe = safe.replace(/`([^`]+)`/g, function (_, code) {
      return stasher.stash(`<code class="math-expr" style="background: rgba(255,255,255,0.06); padding: 0.15rem 0.35rem; border-radius: 4px; font-family: var(--font-mono); font-size: 0.9em; color: var(--accent-teal);">${code}</code>`);
    });
